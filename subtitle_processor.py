...
class SubtitleProcessor:
    ...
    def embed_hard_subtitle(
        ...
        styled_ass_path = temp_dir / f"{subtitle_path.stem}.styled.ass"
        watermark_subtitle_path = temp_dir / "watermark.srt"

        # Convert main subtitle to a basic ASS format
        if not self.convert_srt_to_ass(subtitle_path, ass_subtitle_path):
            raise SubtitleError("Failed to convert SRT to ASS.")

        # Create a new ASS file with the correct style header for Sinhala
        try:
            ass_content = ass_subtitle_path.read_text(encoding='utf-8')
            
            # Define the style for Noto Sans Sinhala
            style_header = """
[Script Info]
; Script generated by Automated Downloader
...
Style: Default,Noto Sans Sinhala,24,&H00FFFFFF,&H000000FF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,2,1,2,10,10,10,1
"""
            # Find where the [Events] section starts and insert the style header before it
            events_section_index = ass_content.find('[Events]')
            if events_section_index != -1:
                final_ass_content = style_header + "\n" + ass_content[events_section_index:]
                styled_ass_path.write_text(final_ass_content, encoding='utf-8')
            else:
                raise SubtitleError("Invalid ASS file format: [Events] section not found.")

        except Exception as e:
            raise SubtitleError(f"Failed to create styled ASS file: {e}")
        ...
        try:
            ...
            for line in process.stdout:
                ...
                if cancel_check and cancel_check():
                    ...
                    if styled_ass_path.exists(): styled_ass_path.unlink()
